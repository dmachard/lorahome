...
void updateDisplay() {
  ...
  if (current_page == 0) {
    ...
    if (idx < active_count) {
      ...
      uint32_t total_pkts = n.packets_count + n.packets_lost;
      float loss_pct = total_pkts > 0 ? (n.packets_lost * 100.0f) / total_pkts : 0.0f;
      det_items[det_count++] = "Lost: " + String(n.packets_lost) + " (" + String(loss_pct, 1) + "%)";
